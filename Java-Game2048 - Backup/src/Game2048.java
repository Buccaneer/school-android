        ...
        for (int line = 0; line < 4; line++)
        {
            ...
            if (direction.equals(Direction.LEFT) || direction.equals(Direction.RIGHT))
            {
                ...
                for (int element = direction.start; direction.check(element, direction.end); element = direction.next(element))
                {
                    ...
                    {
                        if (previous != element && grid[line][element] == grid[line][previous])
                        {
                            grid[line][previous] *= 2;
                            grid[line][element] = 0;
                            previous = pos;
                        }
                        else if (pos != element)
                        {
                            grid[line][pos] = grid[line][element];
                            grid[line][element] = 0;
            ...
            else
            {
                ...
                for (int element = direction.start; direction.check(element, direction.end); element = direction.next(element))
                {
                    if (grid[element][line] != 0)
                    {
                        if (previous != element && grid[element][line] == grid[previous][line])
                        {
                            grid[previous][line] *= 2;
                            grid[element][line] = 0;
                            previous = pos;
                        }
                        else if (pos != element)
                        {
                            grid[pos][line] = grid[element][line];
                            grid[element][line] = 0;
                            previous = previous == pos ? previous : direction.next(previous);
                            pos = direction.next(pos);
                        }
